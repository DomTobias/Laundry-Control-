...
================================ */
    ...
    function carregarDiaAtual() {
        fetch('../config/secagem/hourly_graph/status.php')
            .then(r => r.json())
    ...
    function carregarPorDia(data) {
        fetch(`../config/secagem/hourly_graph/status_dia.php?data=${data}`)
            .then(r => r.json())
            .then(atualizarGrafico)
    ...
    function carregarPorMes(mes, ano) {
        fetch(`../config/secagem/hourly_graph/status_mes.php?mes=${mes}&ano=${ano}`)
            .then(r => r.json())
            .then(atualizarGrafico)
...
const selectAnoApenas = document.getElementById('filtroAnoLinha');
    ...
    function carregarPorAno(ano) {
        fetch(`../config/secagem/hourly_graph/status_ano.php?ano=${ano}`)
            .then(r => r.json())
            .then(atualizarGrafico)
