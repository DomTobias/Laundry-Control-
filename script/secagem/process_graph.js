...
inputDia.addEventListener('change', function() {
    if (this.value) {
        atualizarDados(`../config/secagem/process_graphs/status_dia.php?data=${this.value}`);
    }
});
...
function checkMes() {
    if (selectMes.value && selectAnoMes.value) {
        atualizarDados(`../config/secagem/process_graphs/status_mes.php?mes=${selectMes.value}&ano=${selectAnoMes.value}`);
    }
}
...
selectAnoSolo.addEventListener('change', function() {
    if (this.value) {
        atualizarDados(`../config/secagem/process_graphs/status_ano.php?ano=${this.value}`);
    }
});